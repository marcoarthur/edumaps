-- Revert edumaps:rede_escolas_etapas from pg

BEGIN;

  -- Remove as colunas de etapas, voltando à definição original de
  -- analytics_rede_escolas (sem total_etapas/media_etapas).
  ...
  COMMENT ON MATERIALIZED VIEW analytics.mv_rede_escolas IS 'VIEW MATERIALIZADA - Rede de escolas por município e tipo de administração (federal, estadual, municipal, privada). Agrega matrículas e professores do Censo Escolar 2025 e desempenho IDEB/SAEB (último ano por etapa).';
